...
const registryPath =
  process.env.BAML_PIPELINES_REGISTRY_PATH ??
...
const MIN_CONTEXT_CHARS = Number(
  process.env.BAML_MIN_CONTEXT_CHARS ?? "2000"
);
  ...
  return [
    {
      ...
      description:
        "Execute a registered BAML pipeline with high-context input for external models.",
      inputSchema: {
        type: "object",
        properties: {
          pipeline: {
          ...
          context: {
            type: "string",
            description:
              "Rich narrative context: situation, goal, constraints, what has been tried. Calls with thin context will be rejected.",
            minLength: MIN_CONTEXT_CHARS
          },
          payload: {
            type: "object",
            description:
              "Pipeline-specific JSON payload (see PIPELINES.registry)",
          ...
          options: {
            ...
            properties: {
              ...
              dry_run: {
                type: "boolean",
                description:
                  "If true, only validate pipeline+context; do not execute."
              },
              backend: {
                type: "string",
    ...
    if (context.trim().length < MIN_CONTEXT_CHARS) {
      ...
      return send({
        ...
        error: {
          code: 422,
          message:
            "Context too sparse. Provide a rich description (situation, goal, constraints, tried).",
          data: {
